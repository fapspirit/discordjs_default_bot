...
class DiscordCommandsHandler {
  constructor (token, options = {}) {
    if (!token) throw new Error('DiscordCommandHandler: token is required')
  ...
  on () {
    const commandNames = arguments[0]
    const params = arguments[1]
    const handler = arguments[arguments.length - 1]
    if (!commandNames || !handler) throw new Error('DiscordCommandHandler.on: arguments are missing')
  ...
  listen () {
    return new Promise((resolve, reject) => {
      if (Object.keys(this._commands) === 0) {
        console.warn('DiscordCommandHandler warning: starting without any commands...')
      }
  ...
  _parseCommandNames (name) {
    ...
    if (name instanceof String || name instanceof Number) {
      ...
      throw new Error(`DiscordCommandsHandler: invalid command name value: ${name}`)
    }
...
module.exports = DiscordCommandsHandler
